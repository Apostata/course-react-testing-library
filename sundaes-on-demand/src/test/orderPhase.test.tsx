...
const renderWithRouterAndcontext = (
  ui: ReactNode,
  {
    initialRoute,
    contextProps = {},
  }: { initialRoute: string[]; contextProps?: object }
) => {
  return render(ui as ReactElement<any>, {
    wrapper: (args) =>
      WithWrappersAndRoute({ ...args, initialRoute, contextProps }),
  });
};
...
describe("Making an Order", () => {
  test("happy path, Order Entry to OrderSummary", async () => {
    ...
    submitEvent.preventDefault = jest.mock(``) as unknown as VoidFunction;
  ...
  test("happy path, OrderSummary to ConfirmOrder", async () => {
    ...
    const OrderNumber = await screen.findByText(/order number: \d*/i, {
      exact: false,
    });
  ...
  test("happy path, ConfirmOrder back to Order entry", async () => {
    ...
    renderWithRouterAndcontext(<App />, {
      initialRoute: ["/orderConfirmation"],
      contextProps: { scoops, toppings },
    });
